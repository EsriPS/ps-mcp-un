...
_EXPOSED_CORS_HEADERS = ["mcp-session-id"]
...
def _build_http_middleware() -> list[Middleware]:
    """Build HTTP middleware for browser-based clients.

    By default, local web apps running on ``localhost`` or ``127.0.0.1`` may
    reach the MCP server from any port. Set ``MCP_ALLOWED_ORIGINS`` to a
    comma-separated origin list to override that default.
    """
    raw_allowed_origins = os.getenv("MCP_ALLOWED_ORIGINS", "").strip()
    allow_credentials = os.getenv("MCP_ALLOW_CREDENTIALS", "True").lower() == "true"

    allow_origins: list[str] = []
    allow_origin_regex: str | None = _DEFAULT_CORS_ORIGIN_REGEX

    if raw_allowed_origins:
        allow_origins = [
            origin.rstrip("/")
            for origin in (item.strip() for item in raw_allowed_origins.split(","))
            if origin
        ]
    ...
    if allow_origins == ["*"] and allow_credentials:
        logger.warning(
            "MCP_ALLOWED_ORIGINS='*' is incompatible with credentials; disabling CORS credentials."
        )
    ...
    return [
        Middleware(
            ...
            allow_methods=["*"],
            allow_headers=["*"],
            expose_headers=_EXPOSED_CORS_HEADERS,
        )
    ]

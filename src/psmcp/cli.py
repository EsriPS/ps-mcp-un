...
def _build_parser() -> argparse.ArgumentParser:
    ...
    serve.add_argument(
        "--stateless-http",
        action="store_true",
        help="Use stateless Streamable HTTP mode for clients that do not manage session IDs",
    )
...
def _get_server_config(
    ...
    env_stateless_http = os.getenv("MCP_STATELESS_HTTP")
    if env_stateless_http is None:
        env_stateless_http = os.getenv("FASTMCP_STATELESS_HTTP")
    ...
    stateless_http = getattr(args, "stateless_http", False)
    if not stateless_http and env_stateless_http:
        stateless_http = env_stateless_http.strip().lower() in {"1", "true", "yes", "on"}

    return transport, host, port, ssl_keyfile, ssl_certfile, stateless_http
...
def main(argv: list[str] | None = None):
    ...
    mcp = _init_server()
    transport, host, port, ssl_keyfile, ssl_certfile, stateless_http = _get_server_config(args)
    ...
    if transport == "stdio":
        ...
        middleware = _server._build_http_middleware()
        mcp.run(
            transport=transport,
            host=host,
            port=port,

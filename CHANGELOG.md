...
- **psmcp-router-utilitynetwork**: Consolidated metadata tool (`network_get_metadata`)
  ...
  `categories`, `topology_rules`, `propagators`.
- **psmcp-router-utilitynetwork**: Association query tool (`network_query_associations`)
  for connectivity, containment, and structural attachment queries with full code
  resolution.
- **psmcp-router-utilitynetwork**: Generic trace tool (`network_trace`) supporting
  isolation, connected, subnetwork, subnetworkController, loops, and shortestPath
  trace types with enriched element names.
- **psmcp-router-utilitynetwork**: Workflow orchestration tools:
  `network_downstream_customer_impact`, `network_isolation_analysis`,
  `network_spatial_impact` — multi-step workflows with terminal resolution, trace
  execution, element enrichment, and optional customer data resolution.
- **psmcp-router-utilitynetwork**: Subtype domain resolution tool
  (`network_resolve_coded_values`) for resolving coded attribute values to
  human-readable labels.
- **psmcp-router-utilitynetwork**: MCP resource registrations serving utility network
  guidance (workflow decisions, data model disambiguation, trace interpretation) to
  any MCP client.
- **psmcp-router-utilitynetwork**: Result formatting layer with trace summarization,
  customer impact formatting, and intelligent truncation.

### Changed

- **psmcp-router-utilitynetwork**: Replaced server-side workflow orchestration tools
  (`network_downstream_customer_impact`, `network_isolation_analysis`,
  `network_spatial_impact`) with MCP prompt-based skill workflows. Complex
  multi-step workflows are now guided by prompts that teach the LLM to
  orchestrate atomic tools step-by-step.

### Fixed

### Removed

- **psmcp-router-utilitynetwork**: Removed `network_downstream_customer_impact`,
  `network_isolation_analysis`, and `network_spatial_impact` tools. Replaced by
  prompt-driven skill workflows (`utility_network_downstream_customer_impact`,
  `utility_network_isolation_analysis`, `utility_network_spatial_impact` prompts).

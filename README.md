...
psmcp serve --stateless-http           # stateless HTTP for clients without session handling
...
- `MCP_STATELESS_HTTP` — set to `true` for clients that do not manage Streamable HTTP session IDs
- `MCP_ALLOWED_ORIGINS` — comma-separated browser origins allowed to call the HTTP server
- `MCP_ALLOW_CREDENTIALS` — set to `false` to omit credentialed CORS responses
